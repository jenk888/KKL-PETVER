...
if (is_post()) {
    ...
    if (!$_err) {
        ...
        if ($f) {
            $folder = "../../user_photo";
            $old_photo = "$folder/$photo";
            if (!empty($photo) && file_exists($old_photo)) {
                unlink($old_photo);
            }

            $photo = save_photo($f, $folder);
        }
        $stm = $_db->prepare('UPDATE user SET name = ?, phone= ?, email=? , gender = ?, role=?, photo=? WHERE id = ?');
        $stm->execute([$name,$phone,$email,$gender,$role, $photo,$id]);

        unset($_SESSION['tmp_photo']);
    ...
    <form method="post" class="form-grid" enctype="multipart/form-data">
        ...
        <div class="photo-section">
            ...
            <label class= "upload" tabindex="0">
                <img src="../../user_photo/<?= $photo ?>" alt="Upload Photo" style="width: 100px; height:100px; object-fit: cover; cursor: pointer;">
